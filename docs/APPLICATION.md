...
Numerisect 0.5.0 adds workspace, search, export, scheduling, caching, adapter, and
...
Per-job resource limits are applied to the engine process group before `exec`:
`cpu_seconds` becomes `RLIMIT_CPU`, `memory_mb` becomes `RLIMIT_AS`, and
`wall_seconds` is enforced by a watchdog that cancels the group. Exceeding a limit
fails the job with an explicit `limit_exceeded` reason, and all three values are
recorded in the reproducibility manifest.
