...
| `numerisect_mfactor_kernel.cu` | **Optional** CUDA kernels for the same scan, using Montgomery arithmetic. Built with `nvcc` into `numerisect-mfactor-cuda` when a CUDA toolkit is present, and verified on an RTX 5090 against the published factorizations, PARI/GP and the C helper. It computes nothing the C helper cannot, sieving and testing entirely on the device so no candidate crosses the bus. Measured about twelve times the C helper's rate on an RTX 5090. Two-limb Montgomery carries it to candidates below 2^127; wider ranges go to the C helper, which handles them with GMP rather than deferring them. |
