...
| `numerisect_mfactor_kernel.cu` | **Optional** CUDA kernel for the same scan, using Montgomery arithmetic. Compiled at run time by NVRTC, so no CUDA toolkit is needed, and verified on an RTX 5090 against both the published factorizations and the C helper. It computes nothing the C helper cannot, and measured *slower* than the C helper on a 24-core machine, because the work per candidate is small and moving candidates to the device costs more than testing them in place. Correct and available, but not the fast path. |
